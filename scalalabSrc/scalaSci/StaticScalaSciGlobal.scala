...
/*
 ...
 independent of the library that the interpreter uses.
 ...
 For example the StaticMathsEJML object,  by mixinge the StaticScalaSciGlobal trait,
 ...
 Suppose that we have final defined sin(x: Double) also in scalaSci.EJML.Mat
 ...
 imports the sin() from the scalaSci.EJML.Mat class that is final defined only 
 for scalaSci.EJML.Mat,
 and overwrites the sin() from the scalaSci.EJML.StaticMathsEJML,
 that final defines many more overloaded versions of sin().
...
Fot thar reason at the updated version of ScalaLab we have avoided carefully having duplicate final definitions. 
*/
...
trait  StaticScalaSciGlobal {
  ...
  // final define implicit conversions
     ...
     implicit final def DoubleDoubleArrayToRichDouble1DArray(x: Array[Array[Double]]) =  {  new RichDouble2DArray(x)}  // implicit conversion of an Array[Array[Double]] to RichDouble2DArray
...
covariance(x, x)
 ...
 final def  covariance( v1:Array[Double], v2: Array[Double]) = {    // TODO: check
   var n = v1.length
   var degrees = n-1
