...
// e.g. sin(m) instead of scalaSci.CUDAMat.sin(m)
...
object StaticMathsCUDA  extends AnyRef with scalaSci.StaticScalaSciCommonOps[scalaSci.CUDAMat]  with scalaSci.StaticScalaSciGlobal  {
  ...
  implicit final def MatToDoubleArr(x: scalaSci.Mat) = { x.getRow(0) }
    ...
    def  computeProperMatrixSizeThresholdForMultithreading = {
        ...
        System.gc
        var  testedSize  = 50  // starting value
        
        var breakFlag = false
        var m = rand(testedSize, testedSize)  // create a matrix to test the serial case vs the multithreaded case
...
final def  LU_solve(A:Mat,  b: Mat): Mat = {
    var  LUDec = scalaSci.math.LinearAlgebra.LinearAlgebra.LU(A.v)
    var  jb = new jMatrix(b.v)
...
final def solve(A: Mat,  b: Mat) = {
   new Mat(scalaSci.math.LinearAlgebra.LinearAlgebra.solve( A.v, b.v))
}
...
// sterg-TODO: implement these
  def LU_solve(A: scalaSci.CUDAMat,b: scalaSci.CUDAMat): scalaSci.CUDAMat = {new CUDAMat(1,1)}
  def corr(A: scalaSci.CUDAMat,B: scalaSci.CUDAMat): scalaSci.CUDAMat = {new CUDAMat(1,1)}
  def correlation(A: scalaSci.CUDAMat,B: scalaSci.CUDAMat): scalaSci.CUDAMat = {new CUDAMat(1,1)}
  def cov(A: scalaSci.CUDAMat,B: scalaSci.CUDAMat): scalaSci.CUDAMat = {new CUDAMat(1,1)}
  def covariance(A: scalaSci.CUDAMat,B: scalaSci.CUDAMat): scalaSci.CUDAMat = {new CUDAMat(1,1)}
  def find(A: scalaSci.CUDAMat): Array[Array[Int]] = { Array.ofDim[Int](1,1) }
