...
class Mat( Nin: Int,  Min: Int) extends AnyRef with scalaSci.scalaSciMatrix[scalaSci.MTJ.Mat]  {
  ...
  final def getNativeMatrixRef() =   dm // the scalaSci.MTJ.Mat class wraps the no.uib.cipr.matrix.DenseMatrix class, thus return simply the data representation
  
   // construct and return a scalaSci Mat from the native MTJ DenseMatrix representation
  final def matFromNative() = new Mat(dm) 
  final def matFromNative(dm: DenseMatrix) = new Mat(dm)
