...
class RichDouble2DArray(final var  v: Array[Array[Double]]) extends AnyRef with scalaSci.scalaSciMatrix[scalaSci.RichDouble2DArray]  {
  ...
  final def getNativeMatrixRef() =   v // the scalaSci.RichDouble2DArray does not wrap a Matrix class of a specific library, thus return simply the data representation
...
// we can change the native representation with whatever operation and with this call we can take an updated object   
  final def matFromNative() = new RichDouble2DArray(v)
  final def matFromNative(v: Array[Array[Double]]) = new RichDouble2DArray(v)
...
while (r<sN) {
 ...
 final def * (that: Array[Array[Double]]): RichDouble2DArray =  {
    val productDims = 1.0*Nrows *Ncols*that(0).length
    val useMultithreading = if (productDims >  scalaExec.Interpreter.GlobalValues.mulMultithreadingLimit) true else false
   if (useMultithreading )   {
     if (GlobalValues.hostIsLinux64)
        this jj that   // Linux JBLAS is fast,  use native BLAS
    else
 ...
 override final def * (that: RichDouble2DArray): RichDouble2DArray =  {
    val productDims = 1.0*Nrows *Ncols*that.Ncols
    val useMultithreading = if (productDims >  scalaExec.Interpreter.GlobalValues.mulMultithreadingLimit) true else false
   if (useMultithreading )   {
     if (GlobalValues.hostIsLinux64)
