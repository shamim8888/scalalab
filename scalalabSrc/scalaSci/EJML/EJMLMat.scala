...
final def getNativeMatrixRef() =   sm   // the scalaSci.EJML.EJMLMat wraps an EJML SimpleMatrix
  
final def matFromNative()  = new Mat(sm)
final def matFromNative(sm: SimpleMatrix) = new Mat(sm)
...
final def cc( that: Mat): Mat = {
  var xx = this.getNativeMatrixRef.getMatrix.getData
  var yy = that.getNativeMatrixRef.getMatrix
...
final def *&  (that:  Array[Array[Double]]): Mat = {
   var flmThis = this.getNativeMatrixRef.getMatrix.getData
   var flmThat = scalaSci.RichDouble2DArray.oneDDoubleArray(that)  // construct a FloatMatrix for the argument
...
final def *&  (that:  Mat): Mat = {
   var flmThis = this.getNativeMatrixRef.getMatrix.getData
   var flmThat = that.getNativeMatrixRef.getMatrix.getData
...
*/
  final def ccsolv( b: Array[Double])  = {
    ...
    val ad = this.getNativeMatrixRef.getMatrix.getData
...
final def ccsvd() = {
  ...
  val xx = this.getNativeMatrixRef.getMatrix.getData
